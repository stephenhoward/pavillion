...
Most calendars worth running aren't run by one person. The volunteer who handles the music shows isn't the same person who handles the workshops, and neither of them wants to be the only one with the password. This guide is about adding people to the team — inviting editors, understanding what they can and can't do, removing someone when their role changes, and shaping access in ways that scale from a two-person project to a larger collective without losing track of who's allowed to do what.

A short orientation before the mechanics. Pavillion's editor model is deliberately simple: a calendar has one *owner* (the account that created it) and any number of *editors* (additional accounts the owner has invited in). There are no other tiers — no "contributor," no "moderator," no "publisher-but-not-deleter." That simplicity is on purpose; the cost of a richer permission model is a constant low-grade tax of "wait, can they do this?" every time a new task comes up. The trade-off is that *editor* is a role you should hand out only to people you trust roughly as much as yourself.
...
Click <Btn>Add Editor</Btn>. A small form opens with one field: an email address. Type the email of the person you want to add and submit.
...
- **The email belongs to someone on a different Pavillion instance.** Use the federated form `username@their-instance.example` instead of a plain email. Pavillion looks them up across the federation, adds them as a remote editor, and notifies their instance. Their editing actions on your calendar then flow through ActivityPub the same way any other federated activity does.
...
- **Two-person teams should be two-account teams.** Even if it's just you and a co-organizer, give the co-organizer their own editor account rather than sharing the owner login. The friction is small, and you get an audit trail (who created what), independent passwords, and the ability to remove access without changing a shared password later. See *Shared accounts vs. individual accounts* below.
...
When the team is big enough that the owner doesn't personally know every editor — say, eight or more people, or a mix of long-time members and rotating volunteers — the pattern changes in two ways.
...
A larger collective also tends to want subdivisions ("the music people," "the workshop people") that Pavillion doesn't model. Two ways to handle that:

- **Lean on coordination, not permissions.** If your collective has a Slack, Discord, or matrix room, that's where the "I'm taking the November workshop series" coordination should happen — not in the calendar's permission model. Pavillion gives every editor the same powers; the social layer decides who works on what.
- **Run more than one calendar.** If two parts of your collective publish to genuinely different audiences with little overlap, they probably want different calendars. The music venue's calendar and the kids' programming calendar at the same community center can be two calendars under the same instance, each with the editors who actually work on it. Visitors can follow either or both. (See [When to create a calendar](./when-to-create-a-calendar).)
...
- **No audit trail.** When something changes on the calendar, the system records *which account did it* — but if everyone is logged into the same account, every change traces to "the team," not to a person. The first time something is mistakenly deleted and you want to know who deleted it (so you can talk to them, not punish them), you can't.
...
- **Federation gets weird.** Pavillion records which editor performed each action. A shared account collapses an arbitrary number of people into one "actor" identity, which is confusing for any other calendar that's interacting with yours over federation — the *who* behind a change becomes meaningless.
