...
- The inbound case, out of band: another admin emails you about content hosted on your instance. What to ask them (specifics, links, what they've already done), what to evaluate (is this a CoC issue, a federation-policy issue, or a misunderstanding), what to do (talk to your calendar owner before acting, unless the situation requires otherwise)
- The inbound case, in product: a report filed on another instance and forwarded to yours. This one does not start with you. It lands in the owning calendar's moderation queue first, and reaches your admin queue when the owner acts on it — dismissing a report escalates it to you rather than closing it, so nothing gets buried, but you are the second reader, not the first. The email case above is a different path with different timing, and the two are easy to confuse when both are in flight about the same calendar
