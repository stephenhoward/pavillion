...
```
/inject-standards backend-api                           # All .md files in a skill (except SKILL.md)
/inject-standards backend-api/api                       # Single file from a skill
/inject-standards backend-api/api backend-entity-model  # Multiple targets
```
...
2. **Creating a Skill** — Building a `.agents/skills/` file
...
- If conversation clearly mentions creating a skill, editing `.agents/skills/`, or building a reusable procedure → **Creating a Skill**
...
Read `.agents/skills/standards-routing/index.yml` to get the list of available standards and their descriptions.
...
@.agents/skills/backend-api/api.md
@.agents/skills/global-error-handling/error-handling.md
@.agents/skills/global-conventions/conventions.md
...
When in conversation scenario, check if `.agents/skills/` exists and contains related skills:
...
Arguments can be:
- **Skill name** — `backend-api` → inject all `.md` files in `.agents/skills/backend-api/` (except SKILL.md)
- **Skill/file** — `backend-api/api` → inject `.agents/skills/backend-api/api.md`
...
```
Standard not found: backend-api/nonexistent

Available files in .agents/skills/backend-api/:
- api.md
