...
Discover available skills by listing `.agents/skills/` and reading the frontmatter `description` from each `SKILL.md`. Match skills to the bead based on what the bead's work involves:
...
Discover available standards by reading `.agents/skills/standards-routing/index.yml`. This file lists every standard with a short description. Match standards to the bead based on what the work touches:

```bash
cat .agents/skills/standards-routing/index.yml
```
...
- **Applicable skills:** Discover from `.agents/skills/` frontmatter descriptions
- **Applicable standards:** Discover from `.agents/skills/standards-routing/index.yml`
...
1. **Skills:** List `.agents/skills/` and read each `SKILL.md` frontmatter for its `description` field. Match descriptions to the bead's scope.
2. **Standards:** Read `.agents/skills/standards-routing/index.yml` for the full catalog with descriptions. Match descriptions to the bead's scope.
