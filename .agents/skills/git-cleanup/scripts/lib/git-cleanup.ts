 ...
 * operator-facing flow lives in .agents/skills/git-cleanup/SKILL.md.
 *
...
export function worktreeFamily(path: string): WorktreeFamily {
  ...
  // Agent worktrees are provisioned at `<repo>/.claude/worktrees/`, but
  // `.claude` is a symlink to `.agents`, so a realpath-resolved worktree
  // reports the `.agents` spelling. Both are the same directory.
  if (path.includes('/.claude/worktrees/') || path.includes('/.agents/worktrees/')) return 'agent';
