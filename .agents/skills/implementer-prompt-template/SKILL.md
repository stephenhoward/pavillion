...
escalate (`npx tsx .agents/tools/bead.ts escalate <id> "<reason>"` — the
...
5. If build-guardian passes → proceed to PR. If it fails → retry once,
   then escalate via `npx tsx .agents/tools/bead.ts escalate`.
...
`.agents/commands/spawn-bead-workers.md` (lines 138-176 at extraction
