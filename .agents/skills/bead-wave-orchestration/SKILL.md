...
- [`epic-bead-workflow`](../epic-bead-workflow/SKILL.md) — the needs-human
  escalation protocol (`npx tsx .agents/tools/bead.ts escalate`) used when
  retries exhaust.
...
--maxWorkers=2` — integration files instead need
`--config vitest.integration.config.ts`; see the template skill) and
calls `bd close {bead_id}` only when lint and
...
| **FAIL** | Return findings to the implementer for a single retry round. If a second audit still FAILs, escalate via `npx tsx .agents/tools/bead.ts escalate <id> "<reason>"`. |
...
2. Surface the blocker to the user with options:
   ...
   - Escalate via `npx tsx .agents/tools/bead.ts escalate <id> "<reason>"`
...
- After 2 retries still fail, escalate: call
  `npx tsx .agents/tools/bead.ts escalate <id> "<reason>"`, preserve the
...
`.agents/commands/spawn-bead-workers.md` Phases 3-6 (lines ~120-350).
