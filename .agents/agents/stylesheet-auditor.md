...
Pavillion has a comprehensive design system with CSS custom property tokens, a component class library (`src/client/assets/style/components/`), SCSS mixins, CSS layers, and dark mode support. These conventions are documented in `.agents/skills/stylesheet-playbook/`. Your audit checks whether new or modified styles follow these conventions.
...
Based on the file classifications, read the applicable topic files from `.agents/skills/stylesheet-playbook/`. If changed files involve:
