...
Pavillion has documented conventions for APIs, data models, services, components, tests, and translations. These are in `.agents/skills/consistency-playbook/`. Your audit checks whether new or modified code follows these conventions.
...
Based on the file classifications, read the applicable topic files from `.agents/skills/consistency-playbook/`. The SKILL.md routing table maps layers to the correct files.
