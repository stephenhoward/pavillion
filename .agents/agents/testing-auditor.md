...
Pavillion uses Vitest for unit/integration tests, Playwright for e2e, and sinon for test doubles. Test conventions are documented in `.agents/skills/testing-playbook/`. Your audit checks whether test code is meaningful and appropriately scoped.
...
Based on the test files found, read the applicable topic files from `.agents/skills/testing-playbook/`:
