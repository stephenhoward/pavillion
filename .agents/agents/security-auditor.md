...
2. **Read the playbook on demand.** The standards live in `.agents/skills/security-playbook/`. Start with `SKILL.md` to see what's available, then load only the topic files relevant to the diff:
   ...
   Also read `.agents/skills/review-mode-auditor/SKILL.md` — resolved from the root of the repository checkout under review, not `~/.claude/skills/` — for the shared auditor protocol (constraints, report structure, verdict system).
