...
Pick advisors via the [`agent-discovery`](../skills/agent-discovery/SKILL.md) skill. Pipe the design's **Key Files** (Step 7) one per line into `match-agents.sh advisor`; the JSON array it emits is the advisor set. (The same matcher backs `/shape-spec` and `/process-backlog` so all three commands select advisors identically.) If the array is empty, ask the user whether to proceed without advisory review or revisit the design.

Spawn every matched advisor **in a single Task tool batch** (parallel fan-out per the skill's parallel-spawn pattern). Since no spec files exist for beads, provide the full bead plan inline using this template, adapted per advisor:
...
- **All APPROVE** — Note "Advisory review passed — no concerns flagged" and proceed to Step 10.
