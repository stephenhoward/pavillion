...
Extract tribal knowledge from your codebase into concise, documented standards stored as skills.
...
1. Determine the appropriate skill name. Check if a related skill already exists in `.claude/skills/`:
   - If a matching skill exists, add a companion `.md` file to it
   - If no matching skill exists, create a new skill directory

2. For a **new skill**, create both files:
   - `.claude/skills/<skill-name>/SKILL.md` — with frontmatter description
   - `.claude/skills/<skill-name>/<standard-name>.md` — the standard content

3. For an **existing skill**, add or update the companion `.md` file

4. Draft the content and use AskUserQuestion to confirm:

```
Here's the draft for .claude/skills/backend-api/response-format.md:
...
5. Create or update the file in `.claude/skills/<skill-name>/`
6. **Then repeat Steps 3-4 for the next selected standard**

### Step 5: Update the Index

After all standards are created:

1. Scan `.claude/skills/` for all directories and their `.md` files (excluding SKILL.md)
...
3. Update `.claude/skills/standards-routing/index.yml`:
...
Alphabetize by skill name, then by filename.
...
- .claude/skills/backend-api/response-format.md
- .claude/skills/global-error-handling/error-codes.md
...
All standards: `.claude/skills/<skill-name>/<standard>.md`
Index file: `.claude/skills/standards-routing/index.yml`
...
```
Here's the draft for .claude/skills/backend-api/response-envelope.md:
