...
const reportTranslations: Record<string, string> = {
  ...
  'report.email_placeholder': 'report.email_placeholder',
  'report.email_help': 'report.email_help',
...
const fillAndSubmitForm = async (
  wrapper: VueWrapper,
  overrides: { category?: string; description?: string; email?: string } = {},
) => {
...
describe('ReportEvent Component', () => {
  ...
  describe('Form Rendering', () => {
    ...
    it('should render submit and cancel buttons', async () => {
      ...
      const ghostButtons = wrapper.findAll('.report-dialog__btn--ghost');
    ...
    it('should have accessible label associations on form fields', async () => {
      ...
      expect(emailInput.attributes('id')).toBeTruthy();
      ...
      expect(labelForValues).toContain(emailInput.attributes('id'));
      wrapper.unmount();
    });
  });
  ...
  describe('Client-Side Validation', () => {
    ...
    it('should show error when email is invalid', async () => {
      ...
      await wrapper.find('select').setValue(ReportCategory.SPAM);
      await wrapper.find('textarea').setValue('This event is spam');
      await wrapper.find('input[type="email"]').setValue('not-a-valid-email');
    ...
    it('should not show error when all fields are valid', async () => {
      ...
      expect(wrapper.find('.report-dialog__error').exists()).toBe(false);
      wrapper.unmount();
    });
  });
  ...
  describe('Error State Display', () => {
    ...
    it('should display rate limit error when service throws RateLimitError', async () => {
      const wrapper = mountReportEvent();
      await flushPromises();

      getMockSubmitReport().mockRejectedValueOnce(new RateLimitError());

      await fillAndSubmitForm(wrapper);

      const errorDiv = wrapper.find('.report-dialog__error');
      expect(errorDiv.exists()).toBe(true);
      expect(errorDiv.text()).toBe('report.error_rate_limit');
      wrapper.unmount();
    });
  ...
  describe('Modal Open/Close Behavior', () => {
    ...
    it('should emit close event when cancel button is clicked', async () => {
      ...
      const cancelButtons = wrapper.findAll('.report-dialog__btn--ghost');
