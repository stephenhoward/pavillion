...
describe('CalendarFundingPlanEntity', () => {
  let sampleData: any;

  beforeEach(() => {
    sampleData = {
      id: 'calsub-123',
      funding_plan_id: 'sub-456',
      calendar_id: 'cal-789',
  ...
  it('should create entity with all fields', () => {
    const entity = CalendarFundingPlanEntity.build(sampleData);

    expect(entity.id).toBe(sampleData.id);
    expect(entity.funding_plan_id).toBe(sampleData.funding_plan_id);
  ...
  it('should create entity with future end_time indicating funded through that date', () => {
    ...
    const entity = CalendarFundingPlanEntity.build(data);
  ...
  it('should require funding_plan_id and calendar_id', () => {
    const entity = CalendarFundingPlanEntity.build({
      id: 'calsub-abc',
      funding_plan_id: 'sub-1',
      calendar_id: 'cal-1',
      amount: 100000,
    });

    expect(entity.funding_plan_id).toBe('sub-1');
