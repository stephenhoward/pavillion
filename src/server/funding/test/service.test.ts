...
/**
 * Transaction boundaries around the allocation paths.
 *
 * These run against real rows (no entity stubs on the writes under test) so a
 * rollback is observable as the absence or unchanged state of a database row.
 * Each test injects a failure at the last step of one wrapped path — always a
 * provider call or a write that happens after at least one local write has
 * already been issued — and asserts nothing partial survived.
 */
describe('FundingService - allocation transaction rollback', () => {
  let sandbox: sinon.SinonSandbox;
  let eventBus: EventEmitter;
  let service: FundingService;
  let mockCalendarInterface: {
  ...
  it('should roll back both the end_time write and the plan cancellation when the provider cancel fails', async () => {
    ...
    eventBus.on('funding:plan:cancelled', (payload) => cancelledEvents.push(payload));
