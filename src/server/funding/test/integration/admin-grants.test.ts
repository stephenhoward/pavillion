...
describe('Admin Grant CRUD Integration Tests', () => {
  ...
  describe('GET /api/funding/v1/admin/grants', () => {
    it('should return only active grants when includeRevoked=false (default)', async () => {
      // Create an active grant
      await fundingInterface.createGrant(regularCalendar.id, adminAccount.id, 'active grant');

      // Create a revoked grant
      const grantToRevoke = await fundingInterface.createGrant(adminCalendar.id, adminAccount.id, 'to revoke');
      await fundingInterface.revokeGrant(grantToRevoke.id, adminAccount.id);

      const response = await request(env.app)
        .get('/api/funding/v1/admin/grants')
        .set('Authorization', `Bearer ${adminToken}`);
    ...
    it('should return all grants when includeRevoked=true', async () => {
      ...
      const response = await request(env.app)
        .get('/api/funding/v1/admin/grants?includeRevoked=true')
        .set('Authorization', `Bearer ${adminToken}`);
  ...
  describe('DELETE /api/funding/v1/admin/grants/:id', () => {
    it('should revoke a grant and return 204 No Content', async () => {
      ...
      const response = await request(env.app)
        .delete(`/api/funding/v1/admin/grants/${grant.id}`)
        .set('Authorization', `Bearer ${adminToken}`);
    ...
    it('should return 404 when grant does not exist', async () => {
      ...
      const response = await request(env.app)
        .delete(`/api/funding/v1/admin/grants/${nonExistentId}`)
        .set('Authorization', `Bearer ${adminToken}`);
    ...
    it('should return 400 when grant ID is not a valid UUID', async () => {
      const response = await request(env.app)
        .delete('/api/funding/v1/admin/grants/not-a-valid-uuid')
        .set('Authorization', `Bearer ${adminToken}`);
