...
export default class CalendarAPI {
  ...
  static install(app: Application, internalAPI: CalendarInterface): void {
    ...
    let importSourceRoutes = new ImportSourceRoutes(internalAPI);
    importSourceRoutes.installHandlers(app, '/api/v1');
    ...
    // Admin routes for calendar visibility
    let adminCalendarRoutes = new AdminCalendarRouteHandlers(internalAPI);
    adminCalendarRoutes.installHandlers(app, '/api/v1');
  }
}
