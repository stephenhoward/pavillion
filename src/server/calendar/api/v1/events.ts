...
export default class EventRoutes {
  ...
  installHandlers(app: Application, routePrefix: string): void {
    ...
    router.post(
      ...
      this.cancelEventInstance.bind(this),
    );
    router.delete(
      '/events/:eventId/instances/:instanceId/cancel',
      ExpressHelper.loggedInOnly,
  ...
  /**
   * POST /events/:eventId/instances/:instanceId/cancel
   *
   * Cancel a single materialized instance of a recurring event. Body must
   * include { hideFromPublic: boolean } choosing between EXDATE-style hidden
   * (true) and RECURRENCE-ID-style shown cancellation (false).
   *
   * Returns the updated instance (with isCancelled=true for shown
   * cancellations, or null for hidden cancellations which no longer
   * materialize).
   */
  async cancelEventInstance(req: Request, res: Response) {
    const account = req.user as Account;

    if (!account) {
      res.status(400).json({
        "error": "missing account for instance cancellation. Not logged in?",
        errorName: 'AuthenticationError',
      });
      return;
    }
    ...
    if (!ExpressHelper.isValidUUID(instanceId)) {
      res.status(400).json({
        "error": "invalid UUID format in instance ID",
        errorName: 'ValidationError',
      });
      return;
    }

    const { hideFromPublic } = req.body ?? {};
    ...
    try {
      await this.service.cancelEventInstance(account, eventId, instanceId, hideFromPublic);
      const updated = await this.service.getEventInstanceWithDetails(instanceId);
      res.status(200).json(updated ? updated.toObject() : null);
    }
    catch (error) {
      if (error instanceof ValidationError) {
        ExpressHelper.sendValidationError(res, error);
        ...
        logError(error, "Error cancelling event instance");
        res.status(500).json({
          "error": "An error occurred while cancelling the event instance",
        });
      }
    }
  }
  ...
  /**
   * DELETE /events/:eventId/instances/:instanceId/cancel
   *
   * Restore a previously cancelled instance by removing its exclusion
   * schedule row. No-op if no cancellation exists for the instance.
   */
  async restoreEventInstance(req: Request, res: Response) {
    const account = req.user as Account;

    if (!account) {
      res.status(400).json({
        "error": "missing account for instance restoration. Not logged in?",
        errorName: 'AuthenticationError',
      });
      return;
    }
    ...
    try {
      await this.service.restoreEventInstance(account, eventId, instanceId);
      const updated = await this.service.getEventInstanceWithDetails(instanceId);
      res.status(200).json(updated ? updated.toObject() : null);
    }
    catch (error) {
