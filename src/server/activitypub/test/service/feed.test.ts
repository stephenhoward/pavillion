...
describe("getFeed - local events without EventObjectEntity (seed events)", () => {
  let service: ActivityPubService;
  let sandbox: sinon.SinonSandbox;
  let calendar: Calendar;
  ...
  it('getFeed returns a derived AP URL for local events with no EventObjectEntity record', async () => {
    ...
    // it has no EventObjectEntity record. getFeed() should derive the AP URL from the
    // calendar actor URI so the client can call shareEvent().
    const localEventId = 'seed1234-0000-0000-0000-000000000001';
    const sourceCalendarId = 'source-calendar-uuid-1';
    const sourceActorUri = 'https://local.example.com/calendars/sourcecal';
    ...
    // No EventObjectEntity records for this event
    sandbox.stub(EventObjectEntity, 'findAll').resolves([] as any);

    const feed = await service.getFeed(calendar);

    expect(feed).toHaveLength(1);
    // eventSourceUrl must be derived as actorUri + '/events/' + eventId
    expect(feed[0].eventSourceUrl).toBe(`${sourceActorUri}/events/${localEventId}`);
    expect(feed[0].eventSourceUrl).toMatch(/^https:\/\//);
    expect(feed[0].eventSourceUrl).toContain(localEventId);
  });

  it('getFeed returns empty eventSourceUrl for local events with no CalendarActor either', async () => {
    // Edge case: event is local but no CalendarActorEntity exists for its calendar.
    const localEventId = 'seed5678-0000-0000-0000-000000000002';
    const sourceCalendarId = 'source-calendar-uuid-2';
    ...
    expect(feed).toHaveLength(1);
    // Falls back gracefully when no actor URI is available
    expect(feed[0].eventSourceUrl).toBe('');
  });
});

describe("shareEvent - local events without EventObjectEntity (seed events)", () => {
  let service: ActivityPubService;
  let sandbox: sinon.SinonSandbox;
  let account: Account;
  let calendar: Calendar;
  ...
  it('creates EventObjectEntity on-the-fly for a local event with no AP record (seed event)', async () => {
    // Simulates the repost flow for a seed event.
    // The client sends the synthetic AP URL generated by getFeed() (actorUri + /events/ + eventId).
    // shareEvent() must extract the UUID, find the local event and calendar,
    // create an EventObjectEntity, then proceed with the share.
    const localEventId = 'feedabc0-0000-0000-0000-000000000001';
    const sourceCalendar = Calendar.fromObject({ id: 'source-cal-id', urlName: 'sourcecal' });

    // The actor URI stored in CalendarActorEntity for the source calendar.
    // In the real system this would be https://<config.domain>/calendars/<urlName>.
    // We use the config domain to keep the URL consistent with EventObject.eventUrl().
    const configDomain = 'pavillion.dev'; // matches config/test.yaml
    const actorUri = `https://${configDomain}/calendars/${sourceCalendar.urlName}`;
    // The synthetic URL produced by getFeed() for a seed event
    const syntheticEventUrl = `${actorUri}/events/${localEventId}`;
    // EventObject.eventUrl() will compute the same canonical URL since it uses config domain
    const canonicalApUrl = `https://${configDomain}/calendars/${sourceCalendar.urlName}/events/${localEventId}`;

    // No existing EventObjectEntity for this event
    sandbox.stub(EventObjectEntity, 'findOne').resolves(null);

    // getEventById returns the local event
    const { CalendarEvent } = await import('@/common/model/events');
    const localEvent = new CalendarEvent(localEventId, sourceCalendar.id);
    sandbox.stub(service.calendarService, 'getEventById').resolves(localEvent as any);

    // getCalendar returns the source calendar
    sandbox.stub(service.calendarService, 'getCalendar').resolves(sourceCalendar);

    // actorUrl() is called with the source calendar to get attributed_to
    sandbox.stub(service, 'actorUrl').resolves(actorUri);

    // EventObjectEntity.create should be called to register the AP identity
    const createStub = sandbox.stub(EventObjectEntity, 'create').resolves(
      EventObjectEntity.build({
        id: 'new-eo-id',
        event_id: localEventId,
        ap_id: canonicalApUrl,
    ...
    await service.shareEvent(account, calendar, syntheticEventUrl);

    // Verify EventObjectEntity.create was called with the correct data
    expect(createStub.calledOnce).toBe(true);
    const createArgs = createStub.getCall(0).args[0] as any;
    expect(createArgs.event_id).toBe(localEventId);
    // ap_id must be the canonical AP URL (EventObject.eventUrl() result)
    expect(createArgs.ap_id).toMatch(/^https:\/\//);
    expect(createArgs.ap_id).toContain(localEventId);
    expect(createArgs.ap_id).toContain(sourceCalendar.urlName);
    // attributed_to must be the actor URI from actorUrl()
    expect(createArgs.attributed_to).toBe(actorUri);
  });

  it('still throws InvalidSharedEventUrlError for a URL with no UUID and no EventObjectEntity', async () => {
    // Ensures the fallback only applies when a UUID can be extracted from the URL.
    sandbox.stub(EventObjectEntity, 'findOne').resolves(null);

    await expect(
      service.shareEvent(account, calendar, 'https://remote.com/event/no-uuid-here', false),
    ).rejects.toThrow('Invalid shared event URL');
  });
});
