...
- **Puppeteer MCP server:** to test and verify in-browser behavior of the running application
...
- **Cloud Storage:** Not yet determined, but to be compabible with Flydrive
...
### Federation
- **ActivityPub:** activitypub-express 4.4+ (federation protocol)
- **HTTP Signatures:** http-signature 1.4+ (secure federation)
- **Identity Management:** Custom ActivityPub actor implementation
...
│   ├── locales/        # Client i18n translations
...
│   ├── locales/        # Site i18n translations
│   ├── service/        # Public API services
│   ├── stores/         # Public state stores
│   └── test/           # Site unit tests
...
### Application Hosting
- **Platform:** To be determined (containerized Node.js deployment)
- **Container:** Node.js 22 LTS runtime
- **Process Manager:** PM2 or similar for production
- **Environment Variables:** config library with YAML files

### Database Hosting
- **Production:** PostgreSQL (managed service recommended)
- **Development:** SQLite for testing, PostgreSQL for development
- **Migrations:** Sequelize migrations for schema changes
- **Seeds:** JSON-based seed data for development
