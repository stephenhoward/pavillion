...
- **File:** [decisions/dec-020-widget-embed-policy.md](decisions/dec-020-widget-embed-policy.md)
- **Date:** 2026-09-23 · **Status:** Accepted
- **Decision:** Where a calendar's widget may be framed is the owner's decision, enforced by the visitor's browser through a per-calendar `frame-ancestors` CSP on the widget HTML shell — the universal `frame-ancestors *` is retired, and so is the clickjacking-and-lookup-cost rationale recorded for it in a code comment by commit e42d61ad (a policy correction, not a DEC-to-DEC supersession). The shell is the enforcement point because the iframe's own data calls are same-origin and skip the widget API's Origin check. Every policy permits `'self'` (the admin preview) and `http://localhost:* http://127.0.0.1:* http://*.localhost:*`, production included; a configured domain adds it and its `www` twin over **https only**, any port unless the owner pinned one; **no domain means no embedding elsewhere**; an unknown calendar gets the unconfigured policy (uniformity, not a privacy property); an unbound interface or failed lookup fails closed to `'self'`. `WidgetDomainService.matchesAllowedDomain` is the one host rule for shell and Origin check (case- and trailing-dot-insensitive, the entered host and its `www.` twin only, port significant only if entered) — but the Origin check accepts `http://` where the shell does not, so the two can disagree on scheme. One domain per calendar. Policies are cached 60s (`FRAME_ANCESTORS_CACHE_TTL_MS`), invalidated in-process on save; owner copy says "up to a minute" and moves with the TTL. The header necessarily discloses `widgetAllowedDomain` to anonymous callers — a bounded exception intrinsic to browser enforcement that never licenses adding the field to a public JSON projection; an owner-configuration disclosure, not a [DEC-004](decisions/dec-004-privacy-first-public-access.md) matter. The shell policy is not a funding surface; the [DEC-011](decisions/dec-011-federated-value-boundary.md) `widget_embedding` gate stays on the data endpoint and domain setting.
- **Consult when:** Changing the widget HTML shell's CSP or `X-Frame-Options` headers in `src/server/app_routes.ts`; sending `frame-ancestors *` or any source not derived from the calendar's allowed domain anywhere on the widget path; changing `WidgetDomainService` — `frameAncestorsFor`, `getFrameAncestors`, `matchesAllowedDomain`, `isOriginAllowed`, `setAllowedDomain`, the localhost exception, or `FRAME_ANCESTORS_CACHE_TTL_MS`; changing the widget data API's Origin or `Sec-Fetch-Site` handling; the Allowed Domain admin UI (`widget-domains.vue`, `widget-embed.vue`) or its help copy; supporting more than one domain, wildcard subdomains, or http-only embedding sites; exposing `widgetAllowedDomain` in any public response; adding authenticated state, forms, or any state-changing action to the widget (the permitted frame sources need a clickjacking re-review); deciding whether the widget embed policy should consult funding state.
