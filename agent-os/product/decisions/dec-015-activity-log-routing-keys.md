...
- `object_calendar_id` NULL now carries three distinct meanings — not applicable to this verb, no owning calendar (an admin report against a remote event), or a pre-`0040` row — and none is an error. All three degrade to `target: null`, which is also required by [DEC-004](dec-004-privacy-first-public-access.md): a client must not be able to distinguish "not navigable for this verb" from "not resolvable for you".
