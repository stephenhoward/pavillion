...
**1. Private by default is a property of the endpoint, not of a proxy or a network.** The exposition is served by a **second HTTP listener on a port that compose never publishes**, separate from the application's main listener. Nothing about that privacy depends on a proxy rule, a firewall, or a network topology: it holds identically in the bundled-Caddy (`standalone` profile) deployment, in a bring-your-own-proxy deployment, and on a bare `docker compose up` with no proxy at all. The main listener carries no metrics route, and the unauthenticated `/health` on it stays **liveness only** — up or not up, no per-check detail. The bind address is configurable (`housekeeping.monitoring.metrics.bindAddress`, `METRICS_BIND_ADDRESS`) and defaults to `0.0.0.0`, which is the correct value **inside a container**, where a companion scraper on the project network must reach it and the container boundary is doing the work. Where the process runs directly on a host rather than in a container — development, chiefly — it is pinned to loopback, because an unauthenticated listener on every interface of a developer's laptop is not covered by any compose argument. Neither value is the boundary: no bind address can publish the port, and none is relied on to keep it private.
...
Every value sourced from the housekeeping domain is read through `HousekeepingInterface` ([DEC-003](dec-003-domain-driven-architecture.md)) — the interface's implementation owns the entity and SQL access, and the metrics listener never reaches past it. The one deliberate exception is the media volume: the metrics code reads Media's storage configuration directly — the configured driver, and the base path it locates — and, where that driver is a local volume, `statfs`es that path. Reading the driver is what rule 4 requires of it: an object-storage deployment has no local volume to measure, and emitting a zero there would describe a healthy remote store as a full-to-empty disk, so the driver decides whether the series exists at all. This is a raw OS stat plus the configuration that tells it where and whether to look, with no Media-domain model behind either, so no `MediaInterface` relay method is invented to carry it. That divergence is justified and named here so it is not read as drift; it does not license a second one. The exposition renderer consumes an explicit, named allow-list of fields and never iterates or serializes a raw interface response, so a field added for some other consumer cannot silently become a public series.
...
**4. `pavillion_` is the metric namespace, and documented names are a stable operator contract.** Every series carries the `pavillion_` prefix (`pavillion_backup_*`, `pavillion_disk_*`, `pavillion_db_*`, `pavillion_media_*`, `pavillion_queue_*`, with `pavillion_federation_*` reserved).

Which family a series belongs to is decided by **what the series measures, never by which process measured it**. `pavillion_disk_*` is filesystem usage, keyed by a label identifying the filesystem; **every monitored filesystem belongs to it**, whether the app stats it directly or the worker snapshots it. `pavillion_media_*` is therefore reserved for media-domain quantities that are not filesystem readings — object counts and the like — and `pavillion_backup_*` for backups-as-events: when one last succeeded, how large it was. Without this rule the same physical quantity acquires two names and two shapes, and an operator writing a disk alert has to know which container happened to call `statfs` — which is exactly the boundary the first implementation drew, and the reason the rule is written down. A filesystem measured live has no snapshot behind it and so has no timestamp series for its label value; that is the absent-series rule doing its job, not a gap in the family.

Once a name is published in the operator guide, renaming or removing it requires a deprecation note in that guide. A metric with no underlying data is an **absent series**, never a defaulted zero.
...
This decision is recorded ahead of the code it governs: the `/health` trim has shipped, but the metrics listener, its bind and port configuration, and the exposition itself land with the endpoint work in flight alongside this record. The posture binds that work rather than describing it, which is the point of writing it down first.
